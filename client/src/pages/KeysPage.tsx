...
// Parse newline/comma-separated model IDs entered by the user into a clean array.
  ...
  return (
    <div>
            ...
            <div className="inline-flex gap-1 rounded-xl border p-1">
              {KEYS_TABS.map(tb => (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg transition-colors ${tab === tb.id ? 'bg-foreground text-background font-medium' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                    }`}
                >
                  {t(tb.labelKey)}
                </button>
      ...
      {discoveryStatus.show && (
        <div className={`fixed bottom-6 right-6 z-50 rounded-2xl border px-4 py-3 shadow-lg flex items-center gap-2 max-w-sm animate-in fade-in slide-in-from-bottom-5 duration-300 ${discoveryStatus.type === 'success'
